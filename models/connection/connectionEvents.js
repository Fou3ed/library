...
const ioConnEvents = function () {
  io.on("connection", async (socket) => {
      ...
      try {
        if (onConnectData.user) {
          ...
          Object.entries(socketIds).forEach(([socketId, user]) => {
            if (
              (user.role === "AGENT",
              user.accountId === globalUser[0].accountId)
            ) {
          ...
          console.log("onConnectData", globalUser);
          const profileIds = globalUser.map((user) => user._id.toString());

          if (globalUser[0].role == "AGENT") {
            informConnOperator(
              globalUser.map((agent) => agent.profile_id),
              1
            );
          }
          let accountIds = Object.values(socketIds)
            .filter(
              (user) =>
                user.accountId == globalUser[0].accountId &&
                !(
                  globalUser[0].role == user.role &&
                  globalUser[0].id == user.contactId
                )
            )
            .flatMap((user) => user.userId);
          let updatedConversation = [];
          for (let user of profileIds) {
            let other_profiles = profileIds.filter((uId) => uId !== user);
            updatedConversation.push(
              ...(await putActiveCnvs(user, [...accountIds, ...other_profiles]))
            );
          }
          socket.emit(
            "onConnected",
            onConnectData.type ? globalUser : globalUser[0],
            onConnectData.type ? undefined : globalUser[0]?.balance
          );
          ...
          if (accountIds.length > 0) {
            ...
            const activeConversations =
              globalUser[0].role === "ADMIN"
                ? await getActiveConversationsOwner(globalUser[0].accountId)
            ...
            updatedConversation.forEach(async (conversation) => {
              let members = conversation.member_details
              ...
              let memberSocketId = Object.entries(socketIds)
                .map(([socketId, user]) =>
                  members.find((member) => user.userId.includes(member)) ||
                  conversation.operators.find((admin) =>
                    user.userId.includes(admin.toString())
                  )
                    ? socketId
                    : null
                )
          ...
          if (
            !onConnectData.type &&
            (globalUser[0]?.balance || globalUser[0]?.free_balance)
          ) {
            try {
    ...
    socket.on("operatorConnect", async (agentIds) => {
      operators.push({
        operator: socket.id,
        agentsTeam: agentIds,
      });

      //get the is_active status for every agent in agentIds
      const agentStatus = await getAgentStatus(agentIds);
      console.log("agent status",agentStatus)
      socket.emit("connectionResult", agentStatus);
    });
    ...
    socket.on("disconnecting", async (reason) => {
      ...
      operators = operators.filter(operator => operator.operator !== socket.id);
      await new Promise((resolve) => {
        setTimeout(resolve, 10000);
      });

      if (user) {
        const socketIdsWithUserId = Object.keys(socketIds).filter(
          (socketId) => {
            return (
              socketIds[socketId].role == user.role &&
              socketIds[socketId].contactId == user.contactId
            );
          }
        );
        ...
        let accountIds = Object.values(socketIds)
          .filter(
            (user_) =>
              user_.accountId == user.accountId &&
              !(user_.role == user.role && user_.contactId == user.contactId)
          )
          .flatMap((user) => user.userId);
        let conversations = [];
        ...
        conversations.forEach((conversation) => {
          const room = io
            .of("/")
            .adapter.rooms.get(conversation._id.toString());
          if (room) {
            ...
            if (user) {
              Object.entries(socketIds).forEach(([socketId, user]) => {
                if (
                  (user.role === "ADMIN", user.accountId === user.accountId)
                ) {
                  io.to(socketId).emit(
                    "conversationStatusUpdated",
                    conversation,
        ...
        if (user) {
          ...
          const formattedTimestamp = currentTimestamp.toISOString();
              let profilesArray=[]
          for (let user_ of user.userId) {
          const profileAgent= await putUserActivity({
              userId: user_,
              socketId: socket.id,
              status: false,
              last_seen_at: formattedTimestamp,
            });
            if(profileAgent.role=="AGENT"){
              profilesArray.push(profileAgent.profile_id)

            }
          }
          
        if (profilesArray.length>0) {
          informConnOperator(profilesArray, 0);
        }
      ...
      try {
        check(data.app_id).then((res) => {
          if (res) {
            ...
            logger.info(
              `Event: onConnect ,data: ${JSON.stringify(data)} , socket_id : ${
                socket.id
              } ,token :"" " , date: ${fullDate}" \n `
            );
            ...
            logger.info(
              `Event: Attempt to onConnect ,data: ${JSON.stringify(
                data
              )} , date: ${fullDate}" \n `
            );
    ...
    socket.on("onDisconnect", async (data) => {
      try {
        logger.info(
          `Event: Disconnect ,data: ${JSON.stringify(data)} , socket_id : ${
            socket.id
          } ,token :"" " , date: ${fullDate}"   \n `
        );
    ...
    socket.on("onReconnect", async (data) => {
      try {
        ...
        logger.info(
          `Event: reconnect ,data: ${JSON.stringify(data)} , socket_id : ${
            socket.id
          } ,token :"" " , date: ${fullDate}"   \n `
        );
      } catch (err) {
    ...
    socket.on("contact-form", (data) => {
      try {
        const existingFormIndex = contactForms.findIndex(
          (element) => element.form_id === data.form_id
        );
        if (existingFormIndex !== -1) {
          contactForms[existingFormIndex] = {
            ...contactForms[existingFormIndex],
            ...data,
          };
        } else {
        ...
        logger.info(
          `Event: contact-form, data: ${JSON.stringify(data)}, socket_id: ${
            socket.id
          }, token: "" ", date: ${fullDate}" \n`
        );
      } catch (err) {
